...
#'   as receiver cells. If `NULL`, all cells are used.
#' @param spacex_flag Character scalar naming the metadata column containing
#'   x-coordinates.
#' @param spacey_flag Character scalar naming the metadata column containing
#'   y-coordinates.
#' @param cluster_flag Character scalar naming the cell-annotation metadata
#'   column.
#' @param assay_type Character scalar naming the preferred assay.
#' @param layer_type Character scalar naming the preferred assay layer.
...
#'
#' Construct a weighted sender-neighbor vector for every receiver cell using
#' spatial distances and a selected distance-decay rule.
#'
#' @param locs A data frame containing exactly two spatial-coordinate columns.
#'   Row names must contain the cell identifiers.
#' @param senders Optional character vector of cells eligible to be senders.
#'   Defaults to all cells in `locs`.
#' @param receivers Optional character vector of receiver cells. Defaults to all
#'   cells in `locs`.
#' @param dist_threshold Numeric maximum distance at which a sender is considered
#'   a neighbor.
#' @param self_neighbor Logical; whether a cell may be its own neighbor.
#' @param decay_type Optional character scalar specifying the decay rule. Accepted
#'   abbreviations are `"exp"`, `"rec"`, `"insq"`, and `"rbf"`; other values
#'   use constant weights.
#' @param decay_threshold Numeric relative-weight threshold used to discard weak
#'   neighbors when `normalize = FALSE`.
#' @param normalize Logical; whether each receiver's neighbor weights should sum
#'   to one.
#' @param nclusters Integer number of parallel workers, or `NULL` to choose a
#'   value from the available cores.
#'
#' @return A named list with one numeric weight vector per receiver. Vector names
#'   identify sender cells.
#' @export
Identify_neighbor_list <- function(locs, senders = NULL, receivers = NULL,
                                   dist_threshold = 5, self_neighbor = F,
                                   decay_type = NULL, decay_threshold = 1e-3,
                                   normalize = T, nclusters = 1) {

    if (!is.data.frame(locs) || ncol(locs) != 2) {
        stop("`locs` must be a data frame containing exactly two columns.", call. = FALSE)
    }
    if (is.null(rownames(locs))) {
        stop("`locs` must have non-NULL row names containing cell identifiers.", call. = FALSE)
    }
    ...
    if (is.null(decay_type)) {
        ...
        decay_type <- stringr::str_to_lower(decay_type)
    }
    if (decay_type == "exp") {
    ...
    if (length(intersect(rownames(locs), senders)) == 0) {
        stop("`senders` must contain at least one cell present in the row names of `locs`.", call. = FALSE)
    }
    if (length(intersect(rownames(locs), receivers)) == 0) {
        stop("`receivers` must contain at least one cell present in the row names of `locs`.", call. = FALSE)
    }

    dist_mat <- dist(locs) %>% as.matrix()
    if (!self_neighbor) {
        diag(dist_mat) <- Inf # Will not include self
    }
    ...
    with_progress({
        ...
        if (nclusters > 1) { # We recommend using parallel computing
            ...
            neighbor_list <- future.apply::future_lapply(
                receivers, infer_neighbors_one, future.seed = F
            )
        } else {
...
#'
#' Calculate weighted ligand-receptor communication scores between neighboring
#' sender cells and each receiver cell.
#'
#' @param data A gene-by-cell expression matrix or data frame. Data frames are
#'   converted to matrices. Gene row names and cell column names are required.
#' @param neighbor_list A named list of numeric sender weights, typically returned
#'   by [Identify_neighbor_list()].
#' @param LRDB A data frame containing ligand and receptor genes in columns `From`
#'   and `To`.
#' @param ligands Optional character vector of ligand genes to include.
#' @param receptors Optional character vector of receptor genes to include.
#' @param nclusters Integer number of parallel workers, or `NULL` to choose a
#'   value from the available cores.
#'
#' @return A named list of ligand-receptor score matrices, one per receiver. Rows
#'   are ligand-receptor pairs and columns are sender-receiver pairs.
#' @export
InferCCC <- function(data, neighbor_list, LRDB, ligands = NULL,
                     receptors = NULL, nclusters = 1) {
    if (is.data.frame(data)) {
        data <- as.matrix(data)
    }
    if (is.null(rownames(data))) {
        stop("`data` must have non-NULL row names containing gene identifiers.", call. = FALSE)
    }
    if (is.null(colnames(data))) {
        stop("`data` must have non-NULL column names containing cell identifiers.", call. = FALSE)
    }
    ...
    if (is.null(ligands)) {
        ligands <- LRDB$From %>% unique()
    }
    if (is.null(receptors)) {
        receptors <- LRDB$To %>% unique()
    }
    ligands <- intersect(ligands, allgenes)
    ...
    with_progress({
        ...
        if (nclusters > 1) {
            message("Using parallel computing ... ")
            LR_list <- future.apply::future_lapply(
                receivers, infer_ccc_one, future.seed = F,
...
#' Infer pointwise gene regulatory networks
#'
#' Fit multivariable spatial regressions to estimate regulator-target effects at
#' receiver cells and assess those effects by permutation.
#'
#' @param exps A gene-by-cell expression matrix with gene and cell names.
#' @param neighbor_list A named list of numeric sender weights.
#' @param RTDB A regulator-target data frame whose first two columns identify the
#'   regulator and target and which contains a `PPR` ranking column.
#' @param Target_list Optional character vector restricting genes included in the
#'   regulatory network.
#' @param exp_thres Numeric minimum fraction of receivers in which a gene must be
#'   expressed.
#' @param PPR_top Integer number of top-ranked regulators retained per target.
#' @param rec_only Logical; whether neighbor lists should be restricted to cells
#'   that are also receivers.
#' @param nsim Integer number of response permutations used to estimate p-values.
#' @param lambda Numeric nonnegative spatial smoothing penalty.
#' @param progress Logical; whether to display fitting progress.
#' @param update_every Integer progress-update interval retained for compatibility.
#' @param nclusters Integer number of parallel workers, or `NULL` to choose a
#'   value from the available cores.
#' @param use_delta_y Logical; whether to center each target's expression before
#'   fitting.
#'
#' @return A list containing receiver-by-edge matrices `alpha`, `beta`, and
#'   `pvalue`, transposed so regulatory edges are rows and receivers are columns.
#' @export
InferGRN <- function(exps, neighbor_list, RTDB = NULL, Target_list = NULL,
                     exp_thres = 0.01, PPR_top = 10, rec_only = F, nsim = 99,
                     lambda = 1, progress = TRUE, update_every = 3,
                     nclusters = 1, use_delta_y = T) {
    ...
    results <- fit_multi_spatial_effect(
        data_used, RTDB_used, nl_filtered, nsim, lambda, progress,
        update_every, nclusters, use_delta_y
    )
...
#' Fit multivariable spatial effects
#'
#' Internal regression engine that jointly estimates cell-specific effects for
#' all regulators associated with each target gene.
#'
#' @param data_mat A numeric gene-by-cell expression matrix.
#' @param RTDB_used A data frame with regulator and target columns named `From`
#'   and `To`.
#' @param neighbors A named list of numeric sender weights for receiver cells.
#' @param nsim Integer number of response permutations used for p-values.
#' @param lambda Numeric spatial smoothing penalty.
#' @param progress Logical; whether to display target-level fitting progress.
#' @param update_every Integer progress-update interval retained for compatibility.
#' @param nclusters Integer number of parallel workers, or `NULL` to choose a
#'   value from the available cores.
#' @param use_delta_y Logical; whether to center target expression before fitting.
#'
#' @return A list containing receiver-by-edge matrices `alpha`, `beta`, and
#'   `pvalue`, plus the supplied `lambda` value.
#' @keywords internal
fit_multi_spatial_effect <- function(data_mat, RTDB_used, neighbors, nsim = 99,
                                     lambda = 1, progress = TRUE,
                                     update_every = 50, nclusters = 1,
                                     use_delta_y = FALSE) {
    ...
    for (receiver in receivers) {
        ...
        if (length(wt) == 0 || is.na(receiver_idx)) {
            next
        }
    ...
    alpha <- beta <- pvalue <- matrix(
        NA_real_, nrow = length(receivers), ncol = nrow(RTDB_used),
    ...
    fit_one_target <- function(target) {
        ...
        P <- do.call(
            Matrix::bdiag,
            c(
                list(Matrix::Matrix(0, 1, 1, sparse = TRUE)),
                replicate(p, lambda * L, simplify = FALSE)
            )
        )
        ...
        alpha_part <- beta_part <- pvalue_part <- matrix(
            NA_real_, nrow = length(receivers), ncol = length(edge_idx),
        ...
        if (nsim > 0) {
            perm_counts <- matrix(
                0, nrow = length(receivers), ncol = length(edge_idx),
                dimnames = list(receivers, edge_names[edge_idx])
            )
            valid_counts <- matrix(
                0, nrow = length(receivers), ncol = length(edge_idx),
            ...
            for (i in seq_len(nsim)) {
                ...
                for (pos in seq_along(edge_idx)) {
                    ...
                    perm_counts[valid, pos] <- perm_counts[valid, pos] +
                        (abs(perm_beta[valid]) >= abs(obs_beta[valid]))
        ...
        list(
            edge_idx = edge_idx, alpha = alpha_part,
            beta = beta_part, pvalue = pvalue_part
        )
    }

    total_targets <- length(target_names)
    show_target_progress <- function(done) {
        if (!isTRUE(progress) || total_targets == 0) {
    ...
    if (nclusters > 1 && total_targets > 1) {
        ...
        target_chunks <- split(
            target_names,
            cut(seq_along(target_names), workers, labels = FALSE)
        )
        futures <- lapply(target_chunks, function(target_chunk) {
            future::future(
                lapply(target_chunk, fit_one_target),
                seed = TRUE, packages = c("Matrix")
            )
        })
...
#' Solve a spatial ridge system
#'
#' Internal numerical helper that solves a penalized linear system and retries
#' with progressively stronger diagonal regularization when necessary.
#'
#' @param lhs A square coefficient matrix, usually a `Matrix` sparse matrix.
#' @param rhs A numeric right-hand-side vector or one-column matrix.
#' @param fallback_len Integer length of the missing-value vector returned if all
#'   solve attempts fail.
#'
#' @return A numeric solution vector, or an `NA_real_` vector of length
#'   `fallback_len` when no finite solution can be obtained.
#' @keywords internal
